    ...
    execute: async ({ vendorId, eventDate, guestCount }) => {
      try {
        ...
        if (!session || !session.user || !session.user.id) {
          return {
            success: false,
            error: "User authentication required to create booking",
          };
        }
        ...
        if (vendor.pricePerHead && guestCount) {
          totalPrice = vendor.pricePerHead * guestCount;
        } else if (vendor.packagePrice) {
          totalPrice = vendor.packagePrice;
        }
        ...
        return {
          success: true,
          bookingNumber: newBooking._id.toString(),
          vendorId,
          vendorName: vendor.name,
          category: vendor.category,
          eventDate,
...
Amount: PKR ${totalPrice.toLocaleString()}
